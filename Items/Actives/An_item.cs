...
namespace RPGworldMod
{
    class An_item : SpawnObjectPlayerItem
    {
        ...
        public static void Init()
        {
            ...
            string shortDesc = "Casts A Shadow";

            string longDesc = "Can be thrown. At any point it can cast a shadow very similar to you! Exact match even. Can be used to access closed areas. Telefrags everyone except bosses.\n\n" +
            ...
            GunExt.SetName(item, StringTableManager.GungeonSupportedLanguages.RUSSIAN, "Предмет");
            GunExt.SetShortDescription(item, StringTableManager.GungeonSupportedLanguages.RUSSIAN, "Отбрасывает тень");
            GunExt.SetLongDescription(item, StringTableManager.GungeonSupportedLanguages.RUSSIAN, "Можно бросить. В любой момент может отбросить тень, очень похожую на Вас! Точная копия. Может быть использован для попадания в закрытые помещения. Появление внутри противника моментально убьёт его. Не наносит урон боссам.\n\n" +
            "Просто предмет с тенью. На его месте могло бы быть буквально что угодно кроме, разве что, стекла.");
            ...
            item.ForcedPositionInAmmonomicon = PickupObjectDatabase.GetById(366).ForcedPositionInAmmonomicon;
        }
        ...
        {
            if (user.PlayerHasActiveSynergy("RPGW Green Scarf")) return false;
            return true;
        }
        ...
        private void TeleportEffect(GameObject g, PlayerController p)
        {
            ...
            AkSoundEngine.PostEvent("Play_CHR_ninja_dash_01", base.gameObject);
                    ...
                    p.knockbackDoer.TriggerTemporaryKnockbackInvulnerability(1f);
            GameObject gameObject = TeleportPlayerToCursorPosition.BloodiedScarfPoofVFX;
        ...
        public void ProcessEnemy(AIActor a, float distance)
        {
