...
namespace RPGworldMod
{
    ...
    public class Gruv_machine_gun : GunBehaviour
    {
        ...
        public static void Add()
        {
            ...
            GunExt.SetLongDescription(gun, "Accelerates over time if there are enemies in the room. Resets after dodgerolling.\n\nThe once active «GRUV» corporation produced affordable and high-quality variations of weapons. This machine gun belongs to this product line, having infinite amounts of ammunition. But after the death of the planet Gruv, this antique item is good for nothing.");
            GunExt.SetName(gun, "«GRUV» Machine Gun");
            GunExt.SetName(gun, StringTableManager.GungeonSupportedLanguages.RUSSIAN, "Пулемёт «GRUV»");
            GunExt.SetShortDescription(gun, StringTableManager.GungeonSupportedLanguages.RUSSIAN, "Бесконечный");
            GunExt.SetLongDescription(gun, StringTableManager.GungeonSupportedLanguages.RUSSIAN, "Ускоряется со временем, если владелец в бою. Сбрасывает скорость после переката.\n\nНекогда активная корпорация «GRUV» выпускала доступные и качественные вариации оружия. Этот пулемёт относится к этой линии продукции, обладая бесконечным боезапасом. Но после смерти планеты Грув, этот антикварынй предмет ни на что не годится.");
        ...
        public override void Update()
        {
            ...
            if (gun.DefaultModule.cooldownTime == 0.4f) GunExt.SetAnimationFPS(gun, gun.shootAnimation, 10);
            if (this.PlayerOwner != null && !this.PlayerOwner.PlayerHasActiveSynergy("RPGW King Of The Fourth Chamber") && this.PlayerOwner.HasPickupID(Gruv_machine_gun.ID))
            {
                gun.m_owner = null;
