...
namespace RPGworldMod
{
    ...
    public class Gruv_assault_rifle : GunBehaviour
    {
        ...
        public static void Add()
        {
            ...
            GunExt.SetLongDescription(gun, "Reloading is instant if the magazine is empty.\n\nThe once active «GRUV» corporation produced affordable and high-quality variations of weapons. This assault rifle belongs to this product line, having a distinctive reload speed. But after the death of the planet Gruv, this antique item is good for nothing.");
            GunExt.SetName(gun, "«GRUV» Assault Rifle");
            GunExt.SetName(gun, StringTableManager.GungeonSupportedLanguages.RUSSIAN, "Автомат «GRUV»");
            GunExt.SetShortDescription(gun, StringTableManager.GungeonSupportedLanguages.RUSSIAN, "Быстрая перезарядка");
            GunExt.SetLongDescription(gun, StringTableManager.GungeonSupportedLanguages.RUSSIAN, "Перезарядка моментальна, если магазин пуст.\n\nНекогда активная корпорация «GRUV» выпускала доступные и качественные вариации оружия. Этот автомат относится к этой линии продукции, обладая отличительной скоростью перезарядки. Но после смерти планеты Грув, этот антикварынй предмет ни на что не годится.");
        ...
        {
            if (gun.ClipShotsRemaining <= 0) gun.reloadTime = 0.1f;
            else gun.reloadTime = 1.4f;
            if (this.PlayerOwner != null && !this.PlayerOwner.PlayerHasActiveSynergy("RPGW King Of The Second Chamber") && this.PlayerOwner.HasPickupID(Gruv_assault_rifle.ID))
            {
                gun.m_owner = null;
